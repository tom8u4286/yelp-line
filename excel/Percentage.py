...
for sheet, at in zip(sh_list,at_list):
    for i in range(1,21):
        sheet.conditional_format('B%s:I%s'%(i,i), {'type': 'top','value': 1,"format":format1})
    sheet.write( 1, 1, "higher0Xfrq")
    sheet.write( 1, 0, "percent")
    sheet.write( 1, 1, "higher0Xfrq")
    sheet.write( 1, 2, "higher0Xnfrq")
    sheet.write( 1, 3, "higher0zXnfrq")
    sheet.write( 1, 4, "z_top5")
    sheet.write( 1, 5, "z_top10")
    sheet.write( 1, 6, "cos_avg")
    sheet.write( 1, 7, "sum_higher0")
    sheet.write( 1, 8, "sum_higher05")
    sheet.write( 1, 10, "review_count")
    sheet.write( 1, 11, "menu_length")
    sheet.write( 1, 12, "senti_per_review")
    sheet.write( 1, 13, "dish_senti_avg_len")
    ...
    for r in range(1,21):
        ...
        sheet.write(col, 2, rest_rank_dic["percentage_higher0_cosXnorm_frq"][at])
        sheet.write(col, 3, rest_rank_dic["percentage_higher0_cos_z_Xnorm_frq"][at])
        ...
        sheet.write(col, 9, "rest%s"%rest_num)
        sheet.write(col, 10, rest_dic["review_count"])
        sheet.write(col, 11, rest_dic["menu_length"])
        sheet.write(col, 12, rest_dic["senti_per_review"])
        sheet.write(col, 13, rest_dic["dish_senti_avg_len"])
        col+=1
