...
for sheet, at in zip(sh_list,at_list):
    for i in range(1,23):
        sheet.conditional_format('B%s:G%s'%(i,i), {'type': 'top','value': 1,"format":format1})
    ...
    sheet.write( 1, 5, "cos_sum_total")
    sheet.write( 1, 6, "sum_senti_coo(bl2)")
    #sheet.write( 1, 7, "sum_higher0")
    #sheet.write( 1, 8, "sum_higher05")
    sheet.write( 1, 8, "review_count")
    ...
    for r in range(1,517):
        ...
        sheet.write(col, 6, rest_rank_dic["percentage_sum_senti_coo"][at])
        ...
        sheet.write(col, 7, "rest%s"%rest_num)
        sheet.write(col, 8, rest_dic["review_count"])
