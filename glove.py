...
#@listify
def build_cooccur(vocab, corpus, window_size=4, min_count=None):
    """
...
def get_rest_name():
    rest_num = re.search('[0-9]+',sys.argv[1].split("/")[2]).group(0)
    rest_dic = json.load(open("data/restaurant_dict_list/restaurant_dict_%s.json"%rest_num))
    rest_name = "_"+rest_dic["restaurant_name"].lower().replace(" ","-")
    return rest_name
...
def main(arguments):
    ...
    inv_vocab={}
    with open('data/voc/restaurant_%s_voc.txt'%m.group(0), 'w') as fp:
